		...
		<div class="border-bottom-grey">
			<ul class="bg-white uppercase custom-nav-bars nav nav-tabs align-center font-size-12 no-padding" role="tablist">
				...
				<?php if($node->field_premium_professional['und'][0]['value'] == 1): ?>
					<li class="active"><a href="<?php print  $base_url . '/' . $path . '/affiliate-business'; ?>">Affiliated Business</a></li>
				<?php endif; ?>	
		...
		<div class="container">
			...
			<div class="row clearfix row-eq-height">
				<div class="col-12 col-sm-12 col-xs-12 col-md-3 col-lg-3 col-xl-3 js-sticky-container">
					<div class="js-sticky">
						<div class="bg-white border-radius-3 box-shadow">
							<h3 class="font-size-24 no-margin font-alt fw-600 border-bottom-grey padding-10">Affiliation Type</h3>
							<div class="padding-15 jump-nav-area">
								<?php
									$block = module_invoke('views', 'block_view', '-exp-professional-affiliation');
				...
				<div class="col-12 col-sm-12 col-xs-12 col-md-9 col-lg-9 col-xl-9">
					...
					<?php if((!empty($node->field_affiliate_title)) || (!empty($node->field_affiliate_description)) || (!empty($block))) : ?>
						<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
							<?php if(!empty($node->field_affiliate_title)): ?>
								<?php $title_field = trim(render($node->field_affiliate_title['und']['0']['value'])); ?>
								<?php
							...
							<?php endif; ?>
							<?php if(!empty($node->field_affiliate_description)): ?>
								<?php $description_field = trim(render($node->field_affiliate_description['und'][0]['value'])); ?>
								<?php
					...
					<?php endif; ?>
					<?php
						$block = module_invoke('views', 'block_view', 'professional-affiliation');
						if ($block):
					?>
						...
						<br>
					<?php endif; ?>
					<?php if(!empty($node->field_affiliate_middle_content)): ?>
						<div class="bg-white border-radius-3 box-shadow">
							<?php $middle_content_field = trim(render($node->field_affiliate_middle_content['und'][0]['value'])); ?>
							<?php
					...
					<br>
					<br>
					<?php if(!empty($node->field_affiliate_lower_content)): ?>
						<div class="bg-white border-radius-3 box-shadow">
							<?php $lower_content_field = trim(render($node->field_affiliate_lower_content['und'][0]['value'])); ?>
							<?php

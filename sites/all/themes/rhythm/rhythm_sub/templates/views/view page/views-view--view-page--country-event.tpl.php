			...
			<div class="border-bottom-grey">
				<ul class="bg-white uppercase custom-nav-bars nav nav-tabs align-center font-size-12 no-padding" role="tablist">
					...
					?>
						...
						<?php endif;?>
						<?php if ($item['value'] == "8"): ?>
							<li class="active"><a href="<?php print  $base_url . '/' . $path . '/events' ;?>">Agenda</a></li>
						<?php endif;?>
					<?php endforeach; ?>
				</ul>
			</div>
		...
		<div class="container">
			...
			<div class="row clearfix row-eq-height">
				...
				<div class="col-12 col-sm-12 col-xs-12 col-md-9 col-lg-9 col-xl-9">
					<?php if((!empty($node->field_event_title)) || (!empty($node->field_event_description))) : ?>
						<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
							<?php if(!empty($node->field_event_title)): ?>
								<?php $title_field = trim(render($node->field_event_title['und']['0']['value'])); ?>
								<?php
							...
							<?php endif; ?>
							<?php if(!empty($node->field_event_description)): ?>
								<?php $description_field = trim(render($node->field_event_description['und'][0]['value'])); ?>
								<?php
						...
						<br>
					<?php endif; ?>
					<?php if(!empty($node->field_event_middle_content)): ?>
						<div class="bg-white border-radius-3 box-shadow">
							<?php $middle_content_field = trim(render($node->field_event_middle_content['und'][0]['value'])); ?>
							<?php
					...
					<br>
					<br>
					<?php if(!empty($node->field_event_lower_content)): ?>
						<div class="bg-white border-radius-3 box-shadow">
							<?php $lower_content_field = trim(render($node->field_event_lower_content['und'][0]['value'])); ?>
							<?php
					...
					<?php endif; ?>
					<?php
						$block = module_invoke('views', 'block_view', 'article_unique_content-category_event');
						if ($block):
					?>
	...
	<?php if(!empty($view->style_plugin->rendered_fields)): ?>
		...
		<div class="border-bottom-grey">
			<ul class="bg-white uppercase custom-nav-bars nav nav-tabs align-center font-size-12 no-padding" role="tablist">
				...
				<li><a href="<?php print  $base_url . '/' . $path . '/for-rent'; ?>">For Rent</a></li>
				<li><a href="<?php print  $base_url . '/' . $path . '/experience'; ?>">Experiences</a></li>
				<li class="active"><a href="<?php print  $base_url . '/' . $path . '/events' ;?>">Event Agenda</a></li>
			</ul>
		</div>
		<br>
		<div class="container">
			...
			<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
				<div class="padding-30">
					...
					<?php else: ?>
						<h2 class="no-margin font-alt font-size-24 fw-600">Explore our <?php print $current_year; ?> selection of best <?php print($node->title); ?> events, exhibitions, and shows.</h2>
					<?php endif; ?>	
					<?php if(!empty($node->field_event_description)): ?>
						<?php $description_field = trim(render($node->field_event_description['und'][0]['value'])); ?>
						<?php
					...
					<?php else: ?>
						<br>
						<p>Stay informed with the latest <?php print($node->title); ?> events & exhibitions and subscribe to The Luxe Guide Newsletter:<br>
						- <?php print($node->title); ?> events <br>
						- <?php print($node->title); ?> exhibitions<br>
						- <?php print($node->title); ?> showst<br>
					<?php endif; ?>
				</div>
			</div>
			<br>
			...
			<br>
			<?php
				$official = module_invoke('views', 'block_view', 'professional-pro_official_event');
				$unofficial = module_invoke('views', 'block_view', 'professional-pro_unofficial_event');
			?>
			<?php if($official || $unofficial): ?>
				<h3 class="no-margin font-alt font-size-24 fw-600 border-bottom-grey padding-15"><?php print($node->title); ?> Events</h3>
				<br>
				...
				<?php if($node->field_premium_professional['und'][0]['value'] == 1): ?>
					<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
						...
						<div class="padding-30">
							...
							<br>
							<div class="row clearfix">
								<div class="col-md-3"><a href="<?php print $base_url . '/' . $path . '/events/manila'; ?>">Manila</a></div>
								<div class="col-md-3"><a href="<?php print $base_url . '/' . $path . '/events/cebu-city'; ?>">Cebu</a></div>
							</div>
						</div>
					</div>
					<br>

		...
		<div class="border-bottom-grey">
			<ul class="bg-white uppercase custom-nav-bars nav nav-tabs align-center font-size-12 no-padding" role="tablist">
				...
				?>
					...
					<?php endif;?>
					<?php if ($item['value'] == "4"): ?>
						<li class="active"><a href="<?php print  $base_url . '/' . $path . '/for-sale' ;?>">For Sale</a></li>
					<?php endif;?>
					<?php if ($item['value'] == "5"): ?>
		...
		<div class="row clearfix row-eq-height">
			<div class="col-12 col-sm-12 col-xs-12 col-md-3 col-lg-3 col-xl-3 js-sticky-container">
				<div class="js-sticky">
					...
					<br>
					<?php
						$rent_sub_category_links = module_invoke('views', 'block_view', 'directory-sale_sub_category_link');
			...
			<div class="col-12 col-sm-12 col-xs-12 col-md-9 col-lg-9 col-xl-9">
				<?php $block = module_invoke('views', 'block_view', 'directory-sale_directory'); ?>
				<?php if((!empty($node->field_buy_sell_title)) || (!empty($node->field_buy_sell_description)) || (!empty($block))) : ?>
					<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
						...
						<?php if(!empty($node->field_buy_sell_description)): ?>
							<?php $description_field = trim(render($node->field_buy_sell_description['und'][0]['value'])); ?>
							<?php
						...
						<!-- <div class="padding-15 no-padding-top">
							<div class="row clearfix align-center">
								...
								<?php
									$class_block = module_invoke('views', 'block_view', 'directory-sale_sub_category_link');
									if ($class_block):
								?>
				...
				<?php endif; ?>
				<?php
					$block = module_invoke('views', 'block_view', 'directory-sale_directory');
					if ($block):
				?>
					<?php print render($block['content']); ?>
					<br>
					<?php
						$additional_for_sale_block = module_invoke('views', 'block_view', 'directory-additional_for_sale_category');
						if ($additional_for_sale_block):
					 		print render($additional_for_sale_block['content']);
						endif;
					?>
				...
				<?php if(!empty($node->field_buy_sell_middle_content)): ?>
					<div class="bg-white border-radius-3 box-shadow">
						<?php $middle_content_field = trim(render($node->field_buy_sell_middle_content['und'][0]['value'])); ?>
						<?php
				...
				<?php if(!empty($node->field_buy_sell_lower_content)): ?>
					<div class="bg-white border-radius-3 box-shadow">
						<?php $lower_content_field = trim(render($node->field_buy_sell_lower_content['und'][0]['value'])); ?>
						<?php
				...
				<?php endif; ?>
				<?php
					$block = module_invoke('views', 'block_view', 'article_unique_content-category_sale');
					if ($block):
				?>
					<?php print render($block['content']); ?>
					<br>
	...
	<div class="container">
		...
		<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
			<div class="padding-30">
				...
				<?php else: ?>
					<h2 class="no-margin font-alt font-size-24 fw-600">Explore our <?php print $current_year; ?> selection of best <?php print($node->title); ?> for sale.</h2>
				<?php endif; ?>
				<?php if(!empty($node->field_buy_sell_description)): ?>
					<?php $description_field = trim(render($node->field_buy_sell_description['und'][0]['value'])); ?>
					<?php
				...
				<?php else: ?>
					...
					- Buying <?php print($node->title); ?><br>
					- <?php print($node->title); ?> for sale<br>
					- <?php print($node->title); ?> price<br>
					- <?php print($node->title); ?> Marketplace<br>
					- <?php print($node->title); ?> purchase</p>
				<?php endif; ?>
			</div>
		</div>
		<br>
		...
		<?php if($official || $unofficial): ?>
			<div class="no-margin font-alt font-size-24 fw-600"><?php print($node->title); ?> For Sale</div>
			<br>
...
<?php if($node->type =='models_projects'): ?>
	...
	<div class="container">
		...
		<br>
		<?php
			$sale = module_invoke('views', 'block_view', 'models_projects-all_sale');
		?>
		<?php if($sale): ?>
			<div class="no-margin font-alt font-size-24 fw-600"><?php print($node->title); ?> For Sale</div>
			<br>
			<div><?php print render($sale['content']); ?></div>
			<br>
		<?php endif; ?>
		<!-- Contact Us Entity Form -->
		<div class="bg-white padding-15 border-radius-3 box-shadow">

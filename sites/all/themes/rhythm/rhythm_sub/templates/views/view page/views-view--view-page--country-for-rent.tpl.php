			...
			<div class="border-bottom-grey">
				<ul class="bg-white uppercase custom-nav-bars nav nav-tabs align-center font-size-12 no-padding" role="tablist">
					...
					?>
						...
						<?php endif;?>
						<?php if ($item['value'] == "5"): ?>
							<li class="active"><a href="<?php print  $base_url . '/' . $path . '/for-rent' . '/' . $new_url_title ;?>">For Rent</a></li>
						<?php endif;?>
						<?php if ($item['value'] == "7"): ?>
			...
			<div class="row clearfix row-eq-height">
				<div class="col-12 col-sm-12 col-xs-12 col-md-3 col-lg-3 col-xl-3 js-sticky-container">
					<div class="js-sticky">
						...
						<br>
						<?php
							$rent_sub_category_country_links = module_invoke('views', 'block_view', 'directory-rent_sub_category_country_link');
							if ($rent_sub_category_country_links):
						?>
							<div class="bg-white border-radius-3 box-shadow">
								<div class="font-size-24 no-margin font-alt fw-600 border-bottom-grey padding-15">Type of Rental</div>
								<div class="padding-15 overflow-auto sidenav-class"><ul class="arrow-ul"><?php print render($rent_sub_category_country_links['content']); ?></ul></div>
							</div>
							<br>
							<br>
						<?php endif; ?>
					...
					<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
						...
						<!-- <div class="padding-15">
							<div class="row clearfix align-center">
								...
								<?php
									$class_block = module_invoke('views', 'block_view', 'directory-rent_sub_category_country_link');
									if ($class_block):
								?>
				...
				<br>
				<?php
					$block = module_invoke('views', 'block_view', 'directory-area_rent_directory');
					if ($block):
				?>
					<?php print render($block['content']); ?>
					<br>
					<?php
						$additional_directory_block = module_invoke('views', 'block_view', 'directory-area_addl_rent_category');
				...
				<?php if(!empty($node->field_rent_middle_content)): ?>
					<div class="bg-white border-radius-3 box-shadow">
						<?php $middle_content_field = trim(render($node->field_rent_middle_content['und'][0]['value'])); ?>
						<?php
				...
				<?php
					$block = module_invoke('views', 'block_view', 'article_unique_content-area_category_rent');
					if ($block):
				?>
					<?php print render($block['content']); ?>
					<br>
		...
		<div class="container">
			...
			<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
				<div class="padding-30">
					...
					<?php else: ?>
						<h2 class="no-margin font-alt font-size-24 fw-600">Explore our <?php print $current_year; ?> selection of best <?php print($node->title); ?> for rent in <?php print $country; ?>.</h2>
					<?php endif; ?>
					<?php if(!empty($node->field_rent_description)): ?>
						<?php $description_field = trim(render($node->field_rent_description['und'][0]['value'])); ?>
						<?php
					...
					<?php else: ?>
						...
						- <?php print($node->title); ?> models in <?php print $country; ?><br>
						- <?php print($node->title); ?> price in <?php print $country; ?><br>
						- <?php print($node->title); ?> pricelist in <?php print $country; ?><br>
						- Most expensive <?php print($node->title); ?> in <?php print $country; ?><br>
						- latest <?php print($node->title); ?> models in <?php print $country; ?><br>
						- <?php print($node->title); ?> cost in <?php print $country; ?></p>
					<?php endif; ?>
				</div>
			</div>
			<br>
			...
			<?php if($official || $unofficial): ?>
				<div class="no-margin font-alt font-size-24 fw-600"><?php print($node->title); ?> For Rent <?php print $country; ?></div>
				<br>
				<?php 
				...
				<?php if($node->field_premium_professional['und'][0]['value'] == 1): ?>
					<div class="bg-white no-padding-bottom border-radius-3 box-shadow">
						...
						<div class="padding-30">
							...
							<br>
							<div class="row clearfix">
								<div class="col-md-3"><a href="<?php print $base_url . '/' . $path . '/for-rent/manila'; ?>">Manila</a></div>
								<div class="col-md-3"><a href="<?php print $base_url . '/' . $path . '/for-rent/cebu-city'; ?>">Cebu</a></div>
							</div>
						</div>
					</div>
					<br>
